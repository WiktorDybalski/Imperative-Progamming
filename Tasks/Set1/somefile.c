...
int main(void) 
{
	...
	case 1: // linspace
		if (TEST)
		{
			printf("LINSPACE: Wpisz liczby: n, start, stop ");
		}
	...
	case 2: // add
		if (TEST)
		{
			printf("ADD: Wpisz dlugosc wektorow: ");
		}
		scanf("%d", &len);
		if (len < 0 || len > N)
		{ 
		...
		if (TEST)
		{
			printf("Wpisz elementy pierwszego wektora: ");
		}
		read_vector(vector_1, len);
		if (TEST)
		{
			printf("Wpisz elementy drugiego wektora: ");
		}
	...
	case 3: // dot product
		if (TEST)
		{
			printf("DOT PRODUCT: Wpisz dlugosc wektorow: ");
		}
		scanf("%d", &len);
		if (len < 0 || len > N) 
		{ 
	...
	case 4: // multiply by scalar
		if (TEST)
		{
			printf("MULTIPLY BY SCALAR: Wpisz dlugosc wektora i wartosc skalara: ");
		}
	...
	case 5: // range
		if (TEST)
		{
			printf("RANGE: Wpisz liczby: n, start, step: ");
		}
