...
void bernoulli_gen(int tab[], size_t n, double probab) {
    double win = probab * (double)(RAND_MAX + 1.0);
    for(int i = 0; i < n; i++) {
        tab[i] = (int)(rand() < win);
    }
}
...
void two_dice_probab_distrib(double distrib[], int throws_num) {
    double add = 1.0 / (double)throws_num;
    for(int i = 0; i < throws_num; i++) {
        distrib[rand() % 6 + rand() % 6] += add;
    }
...
void print_histogram(const double tab[], size_t n, int x_start, double y_scale, char mark) {
    for (int i = 0; i < n; i++) {
        int number = i + x_start;
        if (number < 10) {
            printf(" %d| ", number);    
        } else {
            printf("%d| ", number);
        }
        for(int j = 0; j < (int)(tab[i] / y_scale + 0.5); j++)
        {
            printf("%c", mark);
        }
        printf(" %.3f\n", tab[i]);
